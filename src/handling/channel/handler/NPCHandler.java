...
public class NPCHandler {
        ...
        switch (action) {
            ...
            case 1: { // Start Quest
                ...
                if (c.getPlayer().isAdmin() && gui.ZeroMS_UI.ConfigValuesMap.get("脚本显码开关") <= 0) {
                    c.getPlayer().dropMessage(5, "开始系統任务:[scripts/quest/" + quest + "]");
                    c.getPlayer().dropMessage(5, "开始当前NPC:[scripts/npc/: " + npc + "]");
                }
                break;
            }
            case 2: { // Complete Quest
                ...
                if (c.getPlayer().isAdmin() && gui.ZeroMS_UI.ConfigValuesMap.get("脚本显码开关") <= 0) {
                    c.getPlayer().dropMessage(5, "开始系統任务:[scripts/quest/" + quest + "]");
                    c.getPlayer().dropMessage(5, "开始系統任务 NPC:[scripts/npc/: " + npc + "]");
                }
    ...
    public static final void Storage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.hasBlockedInventory2(true)) { //hack
            c.getPlayer().dropMessage(1, "系統錯誤，请联系管理员。");
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        ...
        switch (mode) {
                ...
                if (c.getPlayer().CanStorage()) {
                    if (item != null) {
                        if (!MapleInventoryManipulator.checkSpace(c, item.getItemId(), item.getQuantity(), item.getOwner())) {
                            ...
                            chr.dropMessage(1, "你的物品栏位已经满了..");
                        } else if (chr.getMapId() == 741000209 && chr.getMeso() < 1000) {
                            storage.store(item);
                            chr.saveToDB(false, false);
                            chr.dropMessage(1, "你沒有足够的冒险币放道具。");
                        } else {
                    ...
                    c.getPlayer().dropMessage(1, "操作过快请稍等在试。");
                    storage.store(item);
                }
                break;
            }
            case 5: { // 倉庫處理
                ...
                if (c.getPlayer().CanStorage()) {
                    ...
                    if (chr.getMeso() < 100 || chr.getMapId() == 741000209 && chr.getMeso() < 500) {
                        chr.dropMessage(1, "你沒有足够的冒险币放仓库道具。");
                        break;
                    } else {
                        ...
                        if (item.getItemId() == itemId && (item.getQuantity() >= quantity || GameConstants.isThrowingStar(itemId) || GameConstants.isBullet(itemId))) {
                            ...
                            int cost = (chr.getMapId() == 741000209) ? 500 : 100;
