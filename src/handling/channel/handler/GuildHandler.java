...
public class GuildHandler {
    ...
    private static void SaveCharDb(final MapleCharacter c) {
        try {
            ...
            FileoutputUtil.logToFile("logs/公會存檔保存數據異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getClient().getSession().remoteAddress().toString().split(":")[0] + " 帳號 " + c.getClient().getAccountName() + " 帳號ID " + c.getClient().getAccID() + " 角色名 " + c.getName() + " 角色ID " + c.getId());
            FileoutputUtil.outError("logs/公會存檔保存數據異常.txt", e);
        ...
        switch (operation) {
            ...
            case CREATE: {
                ...
                if (c.getPlayer().getGuildId() > 0 || c.getPlayer().getMapId() != 200000301) {
                    ...
                    c.getPlayer().dropMessage(1, "你沒有足够的冒险币建立公会。目前建立公会需要: " + cost + " 的冒险币。");
                    return;
                }
                final String guildName = slea.readMapleAsciiString();

                if (!isGuildNameAcceptable(guildName)) {
                    c.getPlayer().dropMessage(1, "这个公会名称是不能使用.");
                    return;
                }
            ...
            case LEAVING: {
                ...
                if (c.getPlayer().getMapId() == 990001000) {
                    c.getPlayer().dropMessage(5, "无法在当前地图退出公会。");
                    return;
                }
                ...
                if (ch >= 1) {
                    ...
                    if (victim != null) {
                        if (victim.getMapId() == 990001000) {
                            c.getPlayer().dropMessage(5, "当前无法退出" + victim.getName() + "工會。");
                            return;
                        }
                    }
                }
