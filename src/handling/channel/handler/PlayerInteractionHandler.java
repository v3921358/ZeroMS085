                    ...
                    if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (merchant.isOwner(chr)) {
                                ...
                                chr.dropMessage(1, "这个商店在整理或者是沒在出售道具。");
                            } else if (ips.getFreeSlot() == -1) {
                                chr.dropMessage(1, "商店人数已满，请稍后在进入。");
                            ...
                            if (slea.available() > 0 && slea.readByte() > 0) { //a password has been entered
                                ...
                                if (!pass.equals(ips.getPassword())) {
                                    c.getPlayer().dropMessage(1, "你输入的密码错误,請重新再試一次.");
                                    return;
                                }
                    ...
                    if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
                        ...
                        if (ob instanceof HiredMerchant) {
                            ...
                            if (merchant.isOwner(chr)) {
                                ...
                                chr.dropMessage(1, "这个商店在整理或者是沒在出售物品。");
                            } else if (ips.getFreeSlot() == -1) {
...
//                    }
                    if (ivItem.getQuantity() >= bundles_perbundle) {
                        ...
                        if (GameConstants.isThrowingStar(ivItem.getItemId()) || GameConstants.isBullet(ivItem.getItemId())) {
                            ...
                            FileoutputUtil.logToFile("logs/Data/精靈商人放入道具.txt", FileoutputUtil.NowTime() + "帳號角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + ivItem.getItemId() + " 數量:  " + bundles + "\r\n");
                        } else {
            ...
            case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
                ...
                if (chr.getMeso() - (check2) < 0) {
                    c.getPlayer().dropMessage(1, "您的冒险币不足.");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
                if (shop.getMeso() + (check2) > 2147483647) {
                    c.getPlayer().dropMessage(1, "您购买的商店营业额已经超额，请通知店主来收钱。");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
                }
            ...
            case REMOVE_ITEM: {
                ...
                if (item != null) {
                    if (item.bundles > 0) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/精靈商人道具拿出.txt", FileoutputUtil.NowTime() + "帳號角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 道具： " + item_get.getItemId() + " 數量:  " + item_get.getQuantity() + "\r\n");
                    }
                }
                c.sendPacket(PlayerShopPacket.shopItemUpdate(shop));
                break;
            ...
            case MAINTANCE_ORGANISE: {
                ...
                if (imps != null && imps.getCanShop() && imps.isOwner(chr) && !(imps instanceof MapleMiniGame)) {
                    ...
                    if (chr.getMeso() + imps.getMeso() < 0) {
                        ...
                        FileoutputUtil.logToFile("logs/Data/精靈商人整理道具.txt", FileoutputUtil.NowTime() + "帳號角色名字:" + c.getAccountName() + " " + c.getPlayer().getName() + " 獲得金幣： " + imps.getMeso() + "\r\n");
            ...
            case CLOSE_MERCHANT: {
                try {
                    ...
                    if (merchant != null && merchant.getShopType() == 1 && merchant.isOwner(chr) && merchant.isAvailable()) {
                        ...
                        c.sendPacket(MaplePacketCreator.serverNotice(1, "请去找富兰德里领取你的装备和冒险币"));
                        c.sendPacket(MaplePacketCreator.enableActions());
                    ...
                    FileoutputUtil.outError("logs/精靈商人關閉異常.txt", e);
                }
                break;
                /*  final IMaplePlayerShop merchant = chr.getPlayerShop();
