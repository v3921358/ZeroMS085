...
public class PlayerHandler {
    ...
    public static final void UseChair(final int itemId, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse == null && itemId >= 3010000 && itemId < 9999999) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密语]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") 修改椅子(" + itemId + ")封包，坐上椅子时封锁。 身上并没有该物品"));
...
//        }
        ...
        //int fly = FlyChairDataFactory.getFlyChair(itemId);
        if (GameConstants.isFlyChair(itemId) > 0) { //2019-08-02 修改椅子上飄方式 by Hong
            /*if (fly > 0) {
            final List<Pair<MapleBuffStat, Integer>> stat = Collections.singletonList(new Pair<>(MapleBuffStat.MONSTER_RIDING, 0));
            chr.getClient().getSession().writeAndFlush(MaplePacketCreator.giveMount(GameConstants.isFlyChair(itemId), 0, stat));
    ...
    /*public static final void CancelChair(final short id, final MapleClient c, final MapleCharacter chr) {
        if (id == -1) { // Cancel Chair
          ...
          //  if (/*itemId == 3011000*///map == 749050500 || map == 749050501 || map == 749050502 || map == 741000209) {
    //     chr.cancelFishingTask();
    //  }
    //   chr.setChair(0);
    //   c.getSession().writeAndFlush(MaplePacketCreator.cancelChair(-1));
    //     chr.getMap().broadcastMessage(chr, MaplePacketCreator.showChair(chr.getId(), 0), false);
    //  } else { // Use In-Map Chair
    //     chr.setChair(id);
    //     c.getSession().writeAndFlush(MaplePacketCreator.cancelChair(id));
    // }
    /* final List<MapleBuffStat> bstats = new ArrayList<>();
        bstats.add(0, MapleBuffStat.MONSTER_RIDING);
        chr.getClient().getSession().writeAndFlush(MaplePacketCreator.cancelBuff(bstats));
        //chr.getClient().sendPacket(MaplePacketCreator.cancelBuff(bstats));
        chr.getMap().broadcastMessage(chr, MaplePacketCreator.cancelForeignBuff(chr.getId(), bstats), false);
    }*/
    ...
    public static final void TakeDamage(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (damage == -1) {
            ...
            AutobanManager.getInstance().addPoints(c, 1000, 60000, "不正常的攻击数值，怪物ID:" + monsteridfrom + " : " + damage);
            return;
        }
        if (!chr.inBossMap()) {
        ...
        if (damage > 0) {
            ...
            if (type != -2 && type != -3 && type != -4) {
                ...
                if (chr.getBuffedValue(MapleBuffStat.MAGIC_SHIELD) != null) {
                    damage = (int) (damage - (chr.getBuffedSkill_X(MapleBuffStat.MAGIC_SHIELD) / 100.0) * damage);
                }
            }
...
//			chr.getMap().broadcastMessage(chr, MaplePacketCreator.showMagnet(mobId, slea.readByte()), chr.getPosition());
    ...
    public static final void closeRangeAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr, final boolean energy) {
        ...
        final boolean mirror = chr.getBuffedValue(MapleBuffStat.SHADOWPARTNERX) != null;
    ...
    public static final void DropMeso(final int meso, final MapleCharacter chr) {
        int 丢出金币开关 = gui.ZeroMS_UI.ConfigValuesMap.get("丢出金币开关");
        if (丢出金币开关 > 0) {
...
//            slea.skip(6); //D3 75 00 00 00 00
            ...
            if (portal != null && chr.getMapId() == 211042200 && portal.getName().contains("east00")) {
            ...
            if (targetid != -1 && !chr.isAlive()) {
                ...
                if (divi == 9130401) { // Only allow warp if player is already in Intro map, or else = hack
                    ...
                    if (targetid < 900090000 || targetid > 900090004) { //龙神动画
                        c.sendPacket(UIPacket.IntroDisableUI(false));
                        c.sendPacket(UIPacket.IntroLock(false));
                        c.sendPacket(MaplePacketCreator.enableActions());
                    }
                ...
                c.sendPacket(MaplePacketCreator.enableActions());
                System.out.println("【非法提示】 玩家[" + c.getPlayer().getName() + "]试图以非法形式切换地图！");
            }
        }
        // } catch (Exception ex) {
...
//	slea.readShort(); // Original Y pos
    ...
    public static final void AranCombo(MapleClient c, MapleCharacter chr, int toAdd) {
        if ((chr != null) && (chr.getJob() >= 2000) && (chr.getJob() <= 2112)) {
            ...
            if ((combo > 0) && (curr - chr.getLastCombo() > 6000L)) {//
                combo = 0;
            }
