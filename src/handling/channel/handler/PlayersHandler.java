...
public class PlayersHandler {
    ...
    public static void GiveFame(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (target != null) {
            if (target.getId() == chr.getId()) { // faming self
                ...
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語]  " + chr.getName() + "(" + chr.getId() + ") 修改名聲封包，使用時封鎖。加自己名聲"));
                ...
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + chr.getName() + " 因為修改封包而被管理員永久停權。"));
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語]  " + chr.getName() + "(" + chr.getId() + ")(等級:" + chr.getLevel() + ") 修改名聲封包，使用時封鎖。十五等以下加名聲"));
                chr.ban("修改封包", true, true, false);
                chr.getClient().getSession().close();
                return;
            }
    ...
    public static void RingAction(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (mode == 0) {
            ...
            if (c.getPlayer().getMarriageId() > 0) {
                ...
                c.getPlayer().dropMessage("请先将身上的戒指丢弃掉。");
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            } else if (chr == null) {
                errcode = 0x12;
    ...
    public static void LieDetectorResponse(LittleEndianAccessor slea, MapleClient c) {
        ...
        if (answer.equalsIgnoreCase(ld.getAnswer())) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, new StringBuilder().append("[GM密語] 玩家: ").append(c.getPlayer().getName()).append(" (等級 ").append(c.getPlayer().getLevel()).append(") 通過了測謊儀檢測。").toString()));
        } else if (ld.getAttempt() < 2) {
            ld.startLieDetector(ld.getTester(), ld.getLastType() == 0, true);
        } else {
            MapleCharacter search_chr = c.getPlayer().getMap().getCharacterByName(ld.getTester());
            if ((search_chr != null) && (search_chr.getId() != c.getPlayer().getId())) {
                search_chr.dropMessage(1, new StringBuilder().append(c.getPlayer().getName()).append(" 没有通过测谎仪的检测。").toString());
            ...
            c.getPlayer().changeMap(map, map.getPortal(0));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, new StringBuilder().append("[GM密語] 玩家: ").append(c.getPlayer().getName()).append(" (等級 ").append(c.getPlayer().getLevel()).append(") 未通過測謊儀檢測，疑似使用腳本外掛！").toString()));
        }
    }
