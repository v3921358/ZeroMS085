...
public class InventoryHandler {
    ...
    private static List<IItem> sortItems(final List<IItem> passedMap) {
        ...
        Collections.sort(itemIds); // 对道具ID进行排序
        ...
        if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId) {
           // if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
            if (c.getPlayer().getLevel() > 0) {
                ...
                } else if(NPCScriptManager.getInstance().start_next(c, "items/" + toUse.getItemId())) {
//                    c.removeClickedNPC();
//                    NPCScriptManager.getInstance().start(c, 9310059, 0, "items/" + toUse.getItemId());
                    c.sendPacket(MaplePacketCreator.enableActions());
                } else {
    ...
    public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (toUse.getItemId() == 2050004) {
            ...
            if (nowTimestamp - c.getPlayer().万能疗伤药 < server.Start.ConfigValuesMap.get("万能疗伤药") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
        ...
        if (toUse.getItemId() == 2000004) {
            long nowTimestamp = System.currentTimeMillis();
            if (nowTimestamp - c.getPlayer().特殊药水 < server.Start.ConfigValuesMap.get("特殊药水") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
        ...
        if (toUse.getItemId() == 2000005) {
            long nowTimestamp = System.currentTimeMillis();
            if (nowTimestamp - c.getPlayer().超级药水 < server.Start.ConfigValuesMap.get("超级药水") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
        ...
        if (toUse.getItemId() == 3010100) {
            long nowTimestamp = System.currentTimeMillis();
            if (nowTimestamp - c.getPlayer().财神椅子 < server.Start.ConfigValuesMap.get("财神椅子") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
                chr.dropMessage(5, "还不能使用椅子。");
                return;
            } else {
                c.getPlayer().财神椅子 = nowTimestamp;
            }
        }
        if (toUse.getItemId() == 3010025) {
            long nowTimestamp = System.currentTimeMillis();
            if (nowTimestamp - c.getPlayer().枫叶纪念凳 < server.Start.ConfigValuesMap.get("枫叶纪念凳") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
                chr.dropMessage(5, "还不能使用椅子。");
                return;
            } else {
                c.getPlayer().枫叶纪念凳 = nowTimestamp;
            }
        }
        if (toUse.getItemId() == 3012002) {
            long nowTimestamp = System.currentTimeMillis();
            if (nowTimestamp - c.getPlayer().浴桶椅子 < server.Start.ConfigValuesMap.get("浴桶椅子") * 1000) {
                c.sendPacket(MaplePacketCreator.enableActions());
                chr.dropMessage(5, "还不能使用椅子。");
                return;
            } else {
                c.getPlayer().浴桶椅子 = nowTimestamp;
            }
        }
        if (toUse == null || toUse.getQuantity() < 1 || toUse.getItemId() != itemId) {
            c.sendPacket(MaplePacketCreator.enableActions());
    ...
    public static final void UseReturnScroll(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        c.getPlayer().updateTick(slea.readInt());
        final byte slot = (byte) slea.readShort();
        final int itemId = slea.readInt();//物品ID
        final IItem toUse = chr.getInventory(MapleInventoryType.USE).getItem(slot);
     ...
     * 使用鉴定放大镜
     */
    public static final void UseMagnify(final LittleEndianAccessor slea, final MapleClient c, MapleCharacter chr) {//放大镜处理
        c.getPlayer().updateTick(slea.readInt());
        ...
        final IItem toReveal = (dst < 0) ? c.getPlayer().getInventory(MapleInventoryType.EQUIPPED).getItem(dst) : c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem(dst);
        ...
        final int reqLevel = ii.getReqLevel(eqq.getItemId()) / 20;
        if (eqq.getState() == 1 && (magnify.getItemId() == 2460003 || (magnify.getItemId() == 2460002 && reqLevel <= 12) || (magnify.getItemId() == 2460001 && reqLevel <= 7) || (magnify.getItemId() == 2460000 && reqLevel <= 3))) {
            ...
            final int lines = (eqq.getPotential2() != 0 ? 3 : 2);// 默认2条属性
            ...
            //31001 = 好用的轻功, 31002 = 好用的时空门, 31003 = 好用的火眼晶晶, 31004 = 好用的神圣之火, 41005 = 强化战斗命令, 41006 = 强化进阶祝福, 41007 = 强化极速领域
                for (int i = 0; i < lines; i++) { //最小 2 条, 最大 3 条
                    boolean rewarded = false;
                    while (!rewarded) {
                        StructPotentialItem pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);//载入属性
            ...
            c.getPlayer().forceReAddItem_Flag(toReveal, dst < 0 ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP);
            c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getPotentialReset(c.getPlayer().getId(), eqq.getPosition()));
            ...
            switch (toUse.getItemId()) {
                ...
                case 2430008: // 黄金罗盘
                {
                    ...
                    if (warped) { // Removal of gold compass
                        ...
                        c.getPlayer().dropMessage(5, "全部地都在使用中，请稍后在尝试一次。");
                    }
                    break;
                }
                case 2430112: //神奇魔方碎片
    ...
    public static final void UseTreasureChest(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        if (chr.getInventory(MapleInventoryType.CASH).countById(keyIDforRemoval) > 0) {
            if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.USE).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.SETUP).getNextFreeSlot() > -1 && chr.getInventory(MapleInventoryType.ETC).getNextFreeSlot() > -1) {
                ...
                c.sendPacket(MaplePacketCreator.getShowItemGain(reward, (short) amount, true));
                c.sendPacket(MaplePacketCreator.MonsterBox(toUse.getItemId()));
                //if (rareness > 0) {
                World.Broadcast.broadcastMessage(MaplePacketCreator.getGachaponMega(box, " : 被玩家 “" + c.getPlayer().getName() + "” 幸运抽中！", item, (byte) 1));
                //}
            } else {
                chr.dropMessage(1, "请确保你的背包栏还有空间.");
                c.sendPacket(MaplePacketCreator.enableActions());
            }
        } else {
            chr.dropMessage(1, "请确认是否有" + keyname);
            c.sendPacket(MaplePacketCreator.enableActions());
        }
    }

    @SuppressWarnings("empty-statement")
        public static void UseCashItem(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
...
//        c.getPlayer().updateTick(slea.readInt());
        ...
        switch (itemId) {
            ...
            case 5043000: { //5043000 - NPC瞬移之石 - 完成任务后,可#c直接移动到NPC处#. 对于每个村庄的共有NPC,如冒险岛运营员,星缘,佳佳等NPC无效.
            ...
            case 5520001: { //p.karma
                ...
                if (item != null && !ItemFlag.KARMA_EQ.check(item.getFlag()) && !ItemFlag.KARMA_USE.check(item.getFlag())) {
                    if ((itemId == 5520001 && MapleItemInformationProvider.getInstance().isKarmaEnabled(item.getItemId())) || (itemId == 5520001 && MapleItemInformationProvider.getInstance().isPKarmaEnabled(item.getItemId()))) {
            ...
            case 5062000: { //miracle cube
                ...
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(1, "你的等級不足10级，无法使用。");
                } else {
                    ...
                    if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        ...
                        int stateRate = chr.getClient().getChannelServer().getStateRate();
                        if (eq.getState() >= 5 && eq.getState() != 8) {//解决S级后无法鉴定 8原来是7
                            eq.renewPotential();
                            ...
                            c.getPlayer().dropMessage(5, "该装备没有潜能或已经超过该道具能改变的潜能等级。");
                            fail = true;
                        }
                    } else {
                    ...
                    c.getPlayer().getMap().broadcastMessage(MaplePacketCreator.getPotentialEffect(fail, c.getPlayer().getId(), itemId));
                }
                break;
            }
            case 5062002: { //高级魔方
                ...
                if (c.getPlayer().getLevel() < 100) {
                    c.getPlayer().dropMessage(1, "人物等级在100级之前你不能使用这个 .");
                } else {
                    final Item item = (Item) c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((byte) slea.readInt());
                    if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
                        final Equip eq = (Equip) item;
                        if (eq.getState() >= 5) {
                            eq.renewPotential_super();
                            c.getSession().write(MaplePacketCreator.scrolledItem((Item) toUse, item, false, true));
                            ...
                            c.getPlayer().dropMessage(5, "请确认您要重置的道具具有潜能属性..");
                            fail = true;
                        }
                    } else {
                        c.getPlayer().dropMessage(5, "请检查你的背包是否已满.");
            ...
            case 5570000: { 
                ...
                if (item != null) {
                    if (GameConstants.canHammer(item.getItemId())) {//不能使用的物品
                        final double 加卷次数成功率 = Math.ceil(Math.random() * 100);
                        int 加卷成功率 = server.Start.ConfigValuesMap.get("金锤子提升成功率");
                        if (加卷成功率 >= 加卷次数成功率) {
                            if (server.Start.ConfigValuesMap.get("金锤子提升上限") == 0) {
            ...
            case 5070000: { // Megaphone
                int 游戏喇叭 = gui.ZeroMS_UI.ConfigValuesMap.get("游戏喇叭开关");
                if (游戏喇叭 > 0) {
                    c.getPlayer().dropMessage(1, "目前喇叭功能从后台关闭了，无法广播喇叭");
                    break;
                }
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "必须等級10級以上才可以使用.");
                    break;
                }
                if (!c.getPlayer().getCheatTracker().canAvatarSmega2()) {
                    c.getPlayer().dropMessage(6, "很抱歉为了防止刷广播,所以你每10秒只能用一次.");
                    break;
                }
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/喇叭频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName() + "』频道『" + c.getChannel() + "』喇叭道具『" + itemId + "』：" + message);
                }
                used = true;
            ...
            case 5077000: { // 3 line Megaphone
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/喇叭频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName() + "』频道『" + c.getChannel() + "』喇叭道具『" + itemId + "』：" + messages);
                }
                used = true;
                break;
            }
            case 5072000: { // 高效能喇叭
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/喇叭频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName() + "』频道『" + c.getChannel() + "』喇叭道具『" + itemId + "』：" + message);
                }
                used = true;
                break;
            }
 ...
 /*case 5152100:
            ...
            case 5152107: {
                if ((itemId >= 5152100) && (itemId <= 5152107)) {
                    if (c.getPlayer().getLevel() < 30) {
                        c.getPlayer().dropMessage(1, "必须等級30級以上才可以使用.");
                        break;
                    }
                    int color = (itemId - 5152100) * 100;
                    if (color >= 0) {
            ...
            case 5230000: {// owl of minerva
                    if(ServerConfig.useAutoPot){
                    final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
                    IItem itemm = c.getPlayer().getInventory(MapleInventoryType.EQUIP).getItem((short) 1);
                    if(itemm == null){
                        c.getPlayer().dropMessage(1, "装备栏第一格为空.");
                    } else if (ii.isCash(itemm.getItemId())){
                        c.getPlayer().dropMessage(1, "不支持时装洗潜能.");
                    } else if(!c.getPlayer().haveItem(2460003,8) || !c.getPlayer().haveItem(5062000,8)){
                        c.getPlayer().dropMessage(1, "需要8个最高级放大镜\r\n和8个神奇魔方.");
                    } else {
                        c.getPlayer().gainItem(2460003,-8);
                        c.getPlayer().gainItem(5062000,-8);
                        AbstractPlayerInteraction api = c.getPlayer().getAbstractPlayerInteraction() ;
                        api.sendQuickOwlSearch(itemm);
                        used = true;
                    }
                } else {
            ...
            case 5320000: {
                ...
                if ((Card == null) || (Comment == null)) {
                    ...
                    c.getPlayer().dropMessage(1, "对方必须在线。");
                    c.sendPacket(MaplePacketCreator.enableActions());
                    return;
            ...
            case 5390006:
            case 5390007:
            case 5390008: { // Tiger Roar's Messenger
                 int 游戏喇叭 = gui.ZeroMS_UI.ConfigValuesMap.get("游戏喇叭开关");
                if (游戏喇叭 > 0) {
                    c.getPlayer().dropMessage(1, "目前喇叭功能从后台关闭了，无法广播喇叭");
                    break;
                ...
                if (ServerConfig.LOG_MEGA) {
                    FileoutputUtil.logToFile("logs/聊天/喇叭频道.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家『" + c.getPlayer().getName() + "』频道『" + c.getChannel() + "』喇叭道具『" + itemId + "』：" + text);
                }
                used = true;
            ...
            case 5450000: { // Mu Mu the Travelling Merchant
                ...
                if (c.getPlayer().getLevel() < 10) {
                    c.getPlayer().dropMessage(5, "你还沒有10级以上");
            ...
            default:
                if (itemId / 10000 == 512) {
                    ...
                    if (mesars > 0 && c.getPlayer().getMeso() < (Integer.MAX_VALUE - mesars)) {
                        ...
                        c.getPlayer().gainMeso(mesars, false);
                        c.sendPacket(MTSCSPacket.sendMesobagSuccess(mesars));
                        //} else {
    ...
    public static final void PlayerPickup(final LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
        if (World.isShutDown) {
            c.getPlayer().dropMessage(1, "目前无法捡取物品。");
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
    ...
    public static final void PetPickup(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        try {
            ...
            if (count > range) {// Pet loot Hack
                //    chr.getCheatTracker().registerOffense(CheatingOffense.PET_ITEMVAC_SERVER, " 地圖: " + chr.getMapId() + " 撿物品範圍: " + (long) count);
            }
    ...
    public static final void OWL_CUSTOM(final LittleEndianAccessor slea, final MapleClient c) {
        /*final byte slot = (byte) slea.readShort();
        final int itemid = slea.readInt();
        ...
        c.getPlayer().dropMessage(1,"选择属性成功,请在装备栏查看装备");
        if(c.getPlayer().getAutoPotItems().isEmpty()){
            c.getPlayer().dropMessage(1,"无法多次选择,请关闭该弹窗");
            c.sendPacket(MaplePacketCreator.enableActions());
            return;
        }
        MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.EQUIP, (short)1, (short)1, true);
        final int cid = slea.readInt();
        final int objid = slea.readInt();
        Equip equ = (Equip)c.getPlayer().getAutoPotItems().get(cid);
        MapleInventoryManipulator.addFromDrop(c.getPlayer().getClient(), equ, false);
        c.getPlayer().getAutoPotItems().clear();
        //System.err.println("包:"+slea.toString());
        c.sendPacket(MaplePacketCreator.enableActions());
    }
    ...
    public static final void OwlMinerva(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        if (toUse != null && toUse.getQuantity() > 0 && toUse.getItemId() == itemid && itemid == 2310000) {
            ...
            if (hms.size() > 0) {
                ...
                c.getPlayer().dropMessage(1, "找不到此物品");
            }
        }
        c.sendPacket(MaplePacketCreator.enableActions());
    }
    ...
    public static final void Owl(final LittleEndianAccessor slea, final MapleClient c) {
        if (c.getPlayer().haveItem(5230000, 1, true, false) || c.getPlayer().haveItem(2310000, 1, true, false)) {
            if (c.getPlayer().getMapId() >= 741000209 && c.getPlayer().getMapId() <= 910000022) {
    ...
    public static final void OwlWarp(final LittleEndianAccessor slea, final MapleClient c) {
        if(ServerConfig.useAutoPot){
            ...
            if(itemm == null){
                ...
                AbstractPlayerInteraction api = c.getPlayer().getAbstractPlayerInteraction() ;
                api.sendQuickOwlSearch(itemm);
                api.gainItem(2460003, (short) -8);
                api.gainItem(5062000, (short) -8);
                api.gainItem(5230000, (short) -1);
                c.sendPacket(MaplePacketCreator.enableActions());
                return;
            }
        }
        c.sendPacket(MaplePacketCreator.enableActions());
        if (c.getPlayer().getMapId() >= 741000209 && c.getPlayer().getMapId() <= 910000022 && c.getPlayer().getPlayerShop() == null) {
