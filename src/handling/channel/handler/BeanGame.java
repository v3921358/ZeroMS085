    ...
    public static int[] 消耗品 = {2022140, 2022141, 2022139, 2022137, 1902035, 1902037};*/
        ...
        switch (type) {
            ...
            case 3:
                ...
                c.sendPacket(MaplePacketCreator.BeansJDCS(get进洞次数(), 加速旋转, 蓝, 绿, 红));
                break;
            case 4:
                //记录进洞次数的黄色豆豆。最多只有7个。
                if (get进洞次数() == 0) {
                ...
                c.sendPacket(MaplePacketCreator.BeansJDXZ(get进洞次数(), 第一排, 第三排, 第二排, 启动打怪效果, 中奖率, 加速旋转, 打中女皇出现特效A, 打中女皇出现特效B));
                c.sendPacket(MaplePacketCreator.enableActions());
                if (第二排 != 9) {
                    if (Randomizer.nextInt(Integer.parseInt(黄金狗几率[0])) == Integer.parseInt(黄金狗几率[1]) && 黄金狗设置局数 == 0) {
                        黄金狗设置局数 = 1;
                        c.sendPacket(MaplePacketCreator.BeansHJG((byte) 1));
                    } else {
                        黄金狗设置局数 = 0;
                        c.sendPacket(MaplePacketCreator.BeansHJG((byte) 0));
                    }
                }
                break;
            case 5://应该是普通的怪物中奖 //移动以后三排一样的 这里应该是处理 打完以后出现的 看看是不是判断一排的代码
                ...
                if ((第一排 == 0 && 第三排 == 0 && 第二排 == 0)
                    ...
                    switch (第二排) {
                        ...
                        case 0://大白怪
                            if (Randomizer.nextInt(Integer.parseInt(白怪奖励[0])) > Randomizer.nextInt(Integer.parseInt(白怪奖励[1]))) {
                                ...
                                c.getPlayer().dropMessage(5, "在小鋼珠中獲得：" + experi2 + "經驗值！");
                    ...
                    //chr.gainBeans(獎勵豆豆);
                    chr.modifyJF(4, 奖励豆豆);
                    c.getPlayer().dropMessage(5, "在小鋼珠中獲得: " + 奖励豆豆 + "小鋼珠積分！");
                    // chr.gainExp(1, true, false, true);
                    if (chr.getMapId() == 809030000) {
                        String notea = "恭喜你小鋼珠成功中獎！當前中獲得：" + 奖励豆豆 + "小鋼珠積分！";
                        c.sendPacket(MaplePacketCreator.BeansGameMessage(0x01, 0x01, notea));
                    }
                    if (黄金狗设置局数 > 0 && 第二排 == 9) {
                        黄金狗设置局数 = 0;
                        c.sendPacket(MaplePacketCreator.BeansHJG((byte) 0));
                    }
                    //c.sendPacket(MaplePacketCreator.BeansZJgeidd(true, 奖励豆豆));
                }
                break;
            case 7:
                if (黄金狗设置局数 > 0) {
                ...
                //c.sendPacket(MaplePacketCreator.BeansZJgeidd(true, 0));
                break;
            case 0x0B:
            ...
            default:
                ...
                FileoutputUtil.logToFile("logs/小钢珠未知類型.txt", "時間: " + FileoutputUtil.NowTime() + " IP: " + chr.getClient().getSessionIPAddress() + " MAC: " + chr.getNowMacs() + chr.getName() + "类型【" + type + "】\n包" + slea.toString() + "\r\n");
                break;
        }
        if (type == 0x0B || type == 6) {
            for (int i = 0; i < 5; i++) {
