...
public class MobHandler {
    ...
    public static final void MoveMonster(final LittleEndianAccessor slea, final MapleClient c) {
        ...
        final List<Integer> unk4 = new ArrayList<>();
        final int size = slea.readInt();
        for (int i = 0; i < size; i++) {
            unk4.add(slea.readInt());
        }
        slea.readByte();
        ...
        if (chr.getMapId() != 926013100 && chr.getMapId() != 926013200 && chr.getMapId() != 926013300 && chr.getMapId() != 926013400 && chr.getMapId() != 926013500) {
            try {
                ...
                if (!fly && chr.getMapId() != 921120005 && chr.getMapId() != 921120100 && chr.getMapId() != 921120200 && chr.getMapId() != 921120300 && chr.getMapId() != 921120400 && chr.getMapId() != 921120500 && chr.getMapId() != 921120600) {
                    ...
                    if (((reduce_x > GeneallyDistance_x || reduce_y > GeneallyDistance_y) && reduce_y != 0) || (reduce_x > Check_x && reduce_y == 0) || reduce_x > max_x) {
                        ...
                        if (c.getPlayer().get吸怪() % 50 == 0 || reduce_x > max_x) {
                            //c.getPlayer().getCheatTracker().registerOffense(CheatingOffense.MOB_VAC, "(地圖: " + chr.getMapId() + " 怪物數量:" + chr.get吸怪() + ")");
                            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + chr.getName() + " (編號: " + chr.getId() + ")使用吸怪(" + chr.get吸怪() + ")! - 地圖:" + chr.getMapId() + "(" + chr.getMap().getMapName() + ")"), true);
        ...
        if (res != null) {
            ...
            if (slea.available() != 20) { //9.. 0 -> endPos? -> endPos again? -> 0 -> 0
                System.err.println("slea.available != 20 (movement parsing error)");
            ...
            map.broadcastMessage(chr, MobPacket.moveMonster(useSkill, skill, unk2, monster.getObjectId(), startPos, monster.getPosition(), res, unk4, unk3), monster.getPosition());
        }
