...
import constants.SkillType.俠盜;
import constants.SkillType.冰雷巫師;
import constants.SkillType.劍士;
import constants.SkillType.拳霸;
import constants.SkillType.暗影神偷;
import constants.SkillType.法師;
import constants.SkillType.破風使者3;
import constants.SkillType.神偷;
import constants.SkillType.聖騎士;
import constants.SkillType.聖魂劍士2;
import constants.SkillType.聖魂劍士3;
...
public class DamageParse {
    ...
    public static void applyAttack(final AttackInfo attack, final ISkill theSkill, final MapleCharacter player, int attackCount, final double maxDamagePerMonster, final MapleStatEffect effect, final AttackType attack_type) {
        ...
        if (attack.skill != 0) {
            ...
            if (attack.skill == 4331004) {//幽灵一击 4331004
                last = 15;//攻击次数
            }
            /*修复冲锋队长超能量技能封号*/
            if (attack.skill == 拳霸.元氣彈) {//超能量 5121002
                last = 15;//攻击次数
            }
            /*修复侠盗回旋杀技能封号*/
            if (attack.skill == 俠盜.迴旋斬) {
                last = 15;//攻击次数
            }
            /*修复侠盗金钱炸弹技能封号*/
            if (attack.skill == 神偷.楓幣炸彈) {
                last = 15;//攻击次数
            }
            /*修复风灵使者信天翁技能封号*/
            if (attack.skill == 破風使者3.阿爾法) {
                last = 15;//攻击次数
            }
            /*修复圣骑士连环环破技能封号*/
            if (attack.skill == 聖騎士.騎士衝擊波) {
                last = 15;//攻击次数
            }
            /*修复侠盗一出双击技能封号*/
            if (attack.skill == 神偷.一出双击) {
                last = 15;//攻击次数
            }
            /*修复冰雷雷电术技能封号*/
            if (attack.skill == 冰雷巫師.電閃雷鳴) {
                last = 15;//攻击次数
            }
            /*修复英雄轻舞飞扬技能封号*/
            if (attack.skill == 聖魂劍士3.雙連斬) {
                last = 15;//攻击次数
            }
            /*修复魂骑士灵魂突刺技能封号*/
            if (attack.skill == 聖魂劍士3.靈魂突刺) {
                last = 15;//攻击次数
            }
            /*修复侠盗暗杀技能封号*/
            if (attack.skill == 暗影神偷.致命暗殺) {
                last = 15;//攻击次数
            }
            /*修复战士群体技能封号*/
            if (attack.skill == 劍士.劍氣蹤橫) {
                last = 15;//攻击次数
            }
            /*修复法师魔法双击技能封号*/
            if (attack.skill == 法師.魔力爪) {
                last = 15;//攻击次数
            }
            if (mirror_fix) {
                last *= 15;//全技能默认攻击次数 攻击段数 封号
            }
            if (attack.hits > last) {
        ...
        if (attack.skill == 神偷.楓幣炸彈) { // meso explosion
        ...
        if (attack_type == AttackType.RANGED_WITH_SHADOWPARTNER || attack_type == AttackType.NON_RANGED_WITH_MIRROR) {
            ...
            if (attack_type == AttackType.NON_RANGED_WITH_MIRROR) {
                shadowPartnerEffect = player.getStatForBuff(MapleBuffStat.SHADOWPARTNERX);
            } else {
            ...
            if (monster != null) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                  ...
                  //              World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密语系统] " + player.getName() + " (等级 " + player.getLevel() + ") " + "伤害异常。 " + "最高伤害 40 本次伤害 " + eachd + " 技能ID " + attack.skill));
                        ...
                        boolean ban = false;
                        int atk = 2100000000;//修复龙神假伤原来50W

                        if (!GameConstants.isAran(player.getJob())) {//修复法师破功伤害限制
...
//                                player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
//                                if (eachd > maxDamagePerHit * 2) {
//                                    if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
//                                        if (ServerConfig.LOG_DAMAGE) {
//                                            FileoutputUtil.logToFile("Logs/hack/伤害計算/伤害計算修正_" + monster.getId() + "_" + attack.skill + ".txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 封包伤害 :" + eachd + " 预计伤害 :" + (int) maxDamagePerHit + " 是否为BOSS: " + monster.getStats().isBoss(), false, false);
//                                        }
//                                        player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 预计伤害: ").append((int) maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 技能: ").append(attack.skill).append("]").toString());
//                                    }
//                                    eachd = (int) (maxDamagePerHit * 2); // Convert to server calculated damage
//                                    player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
//                                    if (eachd >= 10000) {
//                                        player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
//                                    }
//                                }
...
//                            if (eachd > maxDamagePerHit * 2.0D && maxDamagePerHit != 1) {
//                                if (ServerConfig.LOG_DAMAGE) {
//                             //       FileoutputUtil.logToFile("Logs/hack/伤害計算/伤害計算修正_" + monster.getId() + "_" + attack.skill + ".txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家: " + player.getName() + "(" + player.getLevel() + ") 職業: " + player.getJob() + " 怪物:" + monster.getId() + " 封包伤害 :" + eachd + " 预计伤害 :" + (int) maxDamagePerHit + " 是否为BOSS: " + monster.getStats().isBoss());
//                                }
//                       //         player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 预计伤害: ").append((int) maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 技能: ").append(attack.skill).append("]").toString());
//                            }
//                        }
//                    }
                ...
                if (range > SkillRange && !player.inBossMap()) { // 815^2 <-- the most ranged attack in the game is Flame Wheel at 815 range
                    player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER, "攻击范围异常,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常范围:" + (int) SkillRange + " 計算范围:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    if (range > SkillRange * 2) {
                        player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER_BAN, "超大攻击范围,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常范围:" + (int) SkillRange + " 計算范围:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    }
                    return;
                }
                // pickpocket
                ...
                if (totDamageToOneMonster > 0) {
                    ...
                    int[] skillsl = {SkillType.夜使者.飛毒殺, 暗影神偷.飛毒殺, SkillType.暗夜行者3.飛毒殺};
                    for (int i : skillsl) {
                    ...
                    switch (attack.skill) {
                        ...
                        case SkillType.暗夜行者3.三飛閃: {
                            ...
                            int[] skills = {SkillType.夜使者.飛毒殺, 暗影神偷.飛毒殺, SkillType.暗夜行者3.飛毒殺};
                            for (int i : skills) {
                                final ISkill skill = SkillFactory.getSkill(i);
                                if (player.getSkillLevel(skill) > 0) {
                                    final MapleStatEffect venomEffect = skill.getEffect(player.getSkillLevel(skill));
        ...
        if (attack.skill == 14111006) {
            effect.applyTo(player, new Point(attack.position.x, attack.position.y)/*attack.positionxy*/);
        }
        if (totDamage > 1) {
                ...
                for (Pair<Integer, Boolean> eachde : oned.attack) {
                    ...
                    if (!GameConstants.isElseSkill(attack.skill)) {
                        ...
                        if ((!GameConstants.isAran(player.getJob()) && player.getLevel() > 10)) {
                            boolean ban = false;
                            if (player.getLevel() <= 20) {//修复法师破功限制
                                atk = 2100000000;
                            } else if (player.getLevel() <= 30) {
                                atk = 2100000000;
                            } else if (player.getLevel() <= 60) {
...
//				    System.out.println("EXCEED!!! Client damage : " + eachd + " Server : " + MaxDamagePerHit);
                                    eachd = (int) (MaxDamagePerHit * 2); // Convert to server calculated damage
                                    player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE_MAGIC_2, new StringBuilder().append("[伤害: ").append(eachd).append(", 預期: ").append(maxDamagePerHit).append(", 怪物: ").append(monster.getId()).append("] [職業: ").append(player.getJob()).append(", 等級: ").append(player.getLevel()).append(", 使用的技能: ").append(attack.skill).append("]").toString());
                                    if (eachd >= 10000) {
                                        if (ServerConfig.LOG_DAMAGE) {
                ...
                if (range > SkillRange && !player.inBossMap()) { // 815^2 <-- the most ranged attack in the game is Flame Wheel at 815 range
                    player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER, "攻击范围异常,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　正常范围:" + (int) SkillRange + " 計算范围:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    if (range > SkillRange * 2) {
                        player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER_BAN, "超大攻击范围,技能:" + attack.skill + "(" + SkillFactory.getName(attack.skill) + ")　怪物:" + monster.getId() + " 正常范围:" + (int) SkillRange + " 計算范围:" + (int) range); // , Double.toString(Math.sqrt(distance))
                    }
                    return;
                }
                if (attack.skill == SkillType.牧师.群體治癒 && !monsterstats.getUndead()) {
                    player.getCheatTracker().registerOffense(CheatingOffense.HEAL_ATTACKING_UNDEAD);
                  //  FileoutputUtil.logToFile("logs/Hack/Ban/技能异常.txt", "\r\n " + FileoutputUtil.NowTime() + " 玩家<" + player.getLevel() + ">: " + player.getName() + " 怪物 " + monster.getId() + " 地圖: " + player.getMapId() + " 技能代码: " + attack.skill + " 使用群體治癒攻击非不死系怪物");
                    World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封锁系统] " + player.getName() + " 因为技能异常而被管理員永久停权。"));
                    World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM 密語系統] " + player.getName() + " (等級 " + player.getLevel() + ") " + "技能异常。 使用群體治癒攻击非不死系怪物 技能ID " + attack.skill));
                    player.ban(player.getName() + "修改WZ",  false,  false, false);
                    player.getClient().getSession().close();
                    return;
                }
                ...
                if ((player.getJob() == 2214) || (player.getJob() == 2215) || (player.getJob() == 2216) || (player.getJob() == 2217) || (player.getJob() == 2218)) {
                    ISkill skill = SkillFactory.getSkill(22141003);
                    if (player.isBuffedValue(22141003)) {
                        MapleStatEffect eff = skill.getEffect(player.getSkillLevel(skill));
                        MonsterStatusEffect monsterStatusEffect = new MonsterStatusEffect(MonsterStatus.SPEED, (int) eff.getX(), skill.getId(), null, false);
                        monster.applyStatus(player, monsterStatusEffect, false, eff.getY() * 2000, monster.getStats().isBoss(), eff);
                    }
                }
            }
        }
        if (attack.skill != SkillType.牧师.群體治癒) {
...
//      }
        ...
        if (elemMaxDamagePerMob > 2100000000) {
    ...
    private static double calculateMaxWeaponDamagePerHit(final MapleCharacter player, final MapleMonster monster, final AttackInfo attack, final ISkill theSkill, final MapleStatEffect attackEffect, double maximumDamageToMonster, final Integer CriticalDamagePercent) {
        ...
        if (theSkill != null) {
            ...
            switch (theSkill.getId()) {
                ...
                case SkillType.神射手.必殺狙擊: // Sniping
                    maximumDamageToMonster = (monster.getStats().isBoss() ? 5000000 : monster.getMobMaxHp());//我多加了个0
                    defined = true; //false关闭破功  true开启破功
                    break;
                case 聖騎士.鬼神之擊://Heavens Hammer
                    maximumDamageToMonster = (monster.getStats().isBoss() ? 500000 : monster.getHp() - 1);
                    defined = true;
                    break;
                case 神偷.楓幣炸彈: // Meso Explosion
        ...
        if (MapleJob.is战神(player.getJob())) {
            maximumDamageToMonster *= 2;
        } else if (MapleJob.is冲锋队长(player.getJob())) {
...
//	}
        ...
        if (elementalMaxDamagePerMonster > 2100000000) {
            if (!defined) {
                elementalMaxDamagePerMonster = 2100000000;
            }
    ...
    public static final AttackInfo Modify_AttackCrit(final AttackInfo attack, final MapleCharacter chr, final int type) {
        ...
        final boolean shadow = (type == 2 && chr.getBuffedValue(MapleBuffStat.SHADOWPARTNER) != null)
                || (type == 1 && chr.getBuffedValue(MapleBuffStat.SHADOWPARTNERX) != null);

        if (attack.skill != 神偷.楓幣炸彈
                && attack.skill != SkillType.狙擊手.寒冰箭
                && attack.skill != SkillType.暗殺者.楓幣攻擊
                && (criticalRate > 0 || attack.skill == 暗影神偷.致命暗殺 || attack.skill == SkillType.神射手.必殺狙擊)) { //blizz + shadow meso + m.e no crits
            ...
            for (AttackPair attackPair : attack.allDamage) {
                if (attackPair.attack != null) {
                    ...
                    for (Pair<Integer, Boolean> eachd : attackPair.attack) {
                        hit++;
                        if (!eachd.right) {
                            if (attack.skill == 暗影神偷.致命暗殺) { //assassinate never crit first 3, always crit last
    ...
    public static final AttackInfo parseDmgMa(final LittleEndianAccessor lea) {
        ...
        lea.skip(8); // ORDER [4] bytes on v.79, [4] bytes on v.80, [1] byte on v.82
        switch (ret.skill) {
    ...
    public static final AttackInfo parseDmgR(final LittleEndianAccessor lea) {
        ...
        switch (ret.skill) {
            ...
            case 破風使者3.暴風神射: // Cygnus Hurricane
