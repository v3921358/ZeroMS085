        ...
        if (validcode) {
            ...
            switch (type) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    MapleInventoryManipulator.addById(c, item, (short) size, "优惠礼品.", null, time);
                    as = 1;
                    break;
                case 4:
    ...
    public static final void BuyCashItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
        ...
        switch (action) {
            ...
            case 3: {   // Buy Item
                ...
                if (canBuy && cItem != null) {
                    ...
                    if (useNX==2&&GameConstants.isCashBuy(cItem.getId())){//抵用券无法购买
                         c.getPlayer().dropMessage(1, "该道具只能通过点券购买。");
                            RefreshCashShop(c);
                            return;
                    }
                    ...
                    if (action == 3) { // 購買單個道具
                     if ((cItem.getId() == 2140003) && (c.getPlayer().getCSPoints(1) >= 100)) {
                    c.getPlayer().modifyCSPoints(1, -100);
                    c.getPlayer().modifyCSPoints(2, 100);
                    c.sendPacket(MaplePacketCreator.serverNotice(1, "兑换100抵用卷成功"));
                     RefreshCashShop(c);
                      return;
                 }    
                        
                        
                 if ((cItem.getId() == 2140004) && (c.getPlayer().getCSPoints(1) >= 500)) {
                ...
                if ((cItem.getId() == 2140005) && (c.getPlayer().getCSPoints(1) >= 1000)) {
                ...
                if ((cItem.getId() == 2140006) && (c.getPlayer().getCSPoints(1) >= 2000)) {
                    c.getPlayer().modifyCSPoints(1, -2000);
                    c.getPlayer().modifyCSPoints(2, 2000);
                    c.sendPacket(MaplePacketCreator.serverNotice(1, "兑换抵2000用卷成功"));
                     RefreshCashShop(c);
                      return;
                } 
                    if ((cItem.getId() == 2140007) && (c.getPlayer().getCSPoints(1) >= 5000)) {
                   ...
                   if ((cItem.getId() == 2140008) && (c.getPlayer().getCSPoints(1) >= 5000)) {
                    c.getPlayer().modifyCSPoints(1, -10000);
                    c.getPlayer().modifyCSPoints(2, 10000);
                    c.sendPacket(MaplePacketCreator.serverNotice(1, "兑换10000抵用卷成功"));
                     RefreshCashShop(c);
                      return;
                }  
                    /*else {
            ...
            case 4: { // gift
                ...
                if (canBuy && info != null && cItem != null) {
                    ...
                    chr.sendNote(characterName, chr.getName() + " 送了你礼物! 赶快去商城收取.", (byte) 0); //fame or not
            ...
            case 6: {
                ...
                if (coupon) {
                    ...
                    if (chr.getCSPoints(useNX) >= (ServerConfig.DISCOUNTED ? 540 : 600) && chr.getInventory(type).getSlotLimit() < 89) {
                        ...
                        chr.dropMessage(1, "栏位已扩充到 " + chr.getInventory(type).getSlotLimit());
                        if (ServerConfig.LOG_CSBUY) {
            ...
            case 8: {
                ...
                if (c.gainCharacterSlot()) {
                    ...
                    chr.dropMessage(1, "角色栏位已扩充到 " + c.getCharacterSlots());
                    if (ServerConfig.LOG_CSBUY) {
        ...
        case 27: { //換購
            ...
            case 32: { // Package gift
                ...
                if (c.getSecondPassword() != null) {
                    ...
                    if (info == null || info.getLeft().intValue() <= 0 || info.getLeft().intValue() == c.getPlayer().getId() || info.getRight().getLeft() == c.getAccID()) {
                        ...
                        c.getPlayer().modifyCSPoints(1, -cItem.getPrice(), false);
                        c.sendPacket(MTSCSPacket.sendGift(characterName, cItem, cItem.getPrice() / 2, false));
                        chr.sendNote(characterName, chr.getName() + " 送了你陆武! 赶快去商城确认看看.", (byte) 0); //fame or not
                        MapleCharacter receiver = c.getChannelServer().getPlayerStorage().getCharacterByName(characterName);
                        if (receiver != null) {
                            receiver.showNote();
                        }
            ...
            case 51: { //楓葉點數購買
                ...
                chr.dropMessage(1, "成功兑换抵用卷:" + item.getPrice());
