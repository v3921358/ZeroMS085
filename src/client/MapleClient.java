...
public class MapleClient {
    ...
    public static final int DEFAULT_CHARSLOT = 1;
    ...
    private List<CharNameAndId> loadCharactersInternal(int serverId) {
        ...
        try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("SELECT id, name FROM characters WHERE accountid = ? AND world = ?")) {
    ...
    public final void update2ndPassword() {
        try {
            ...
            try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE `accounts` SET `2ndpassword` = ? WHERE id = ?")) {
                ps.setString(1, hash);
    ...
    public final void updateLoginState(final int newstate, final String SessionID) { // TODO hide?
        ...
        loginMutex.lock();
        try {
            try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("UPDATE accounts SET loggedin = ?, SessionIP = ?, lastlogin = CURRENT_TIMESTAMP() WHERE id = ?")) {
    ...
    public final byte getLoginState() { // TODO hide?
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            loggedIn = false;
            FileoutputUtil.outError("logs/資料庫異常.txt", e);
            throw new DatabaseException("登录状态获取失败", e);
        }
    }
        ...
        try {
          ...
          //  }
            ...
            if (player.getEventInstance() != null) {
                player.getEventInstance().playerDisconnected(player, player.getId());//v085
            }
 ...
 //               player.getMap().removePlayer(player);
             //  player.getMap().removePlayer(player);//离线挂机
               if(gui.ZeroMS_UI.ConfigValuesMap.get("离线挂机开关") == 0){
    ...
    public boolean canClickNPC() {
        return lastNpcClick + 1200 < System.currentTimeMillis();
    }
    ...
    public void setVip(int x) {
        try (Connection con = DatabaseConnection.getConnection()) {
            ...
            System.err.println("[vip]无法连接数据库");
            FileoutputUtil.outError("logs/資料庫異常.txt", ex);
        } catch (Exception ex) {
            FilePrinter.printError("MapleCharacter.txt", ex, "SetVip");
    ...
    public static boolean banMacs(String macData) {
        ...
        try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO macbans (mac) VALUES (?)")) {
    ...
    public boolean isBannedMac(String mac) {
        ...
        boolean ret = false;
        try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM macbans WHERE mac = ?")) {
            ps.setString(1, mac);
            try (ResultSet rs = ps.executeQuery()) {
                rs.next();
                if (rs.getInt(1) > 0) {
    ...
    private void loadMacsIfNescessary() throws SQLException {
        if (macs.isEmpty()) {
            try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("SELECT macs FROM accounts WHERE id = ?")) {
                ps.setInt(1, accountId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
    ...
    /*public static boolean landersLogin(String lip) {
        ...
        boolean ret = false;
        try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("SELECT COUNT(*) FROM ipcheck WHERE ? LIKE CONCAT(ip, '%')")) {
            ps.setString(1, ip);
            try (ResultSet rs = ps.executeQuery()) {
                rs.next();
                if (rs.getInt(1) > 0) {
    ...
    public final void unLockDisconnect() {
        getSession().writeAndFlush(MaplePacketCreator.serverNotice(1, "当前账号在别处登录\r\n若不是你本人操作请及时更改密码。"));
