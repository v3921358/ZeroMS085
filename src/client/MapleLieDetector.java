...
public class MapleLieDetector {
    ...
    public final boolean startLieDetector(final String tester, final boolean isItem, final boolean anotherAttempt) {
            if (!ServerConfig.测谎仪开关) {
            final MapleCharacter searchchr = MapleCharacter.getOnlineCharacterById(this.cid);
            NPCScriptManager.getInstance().start(searchchr.getClient(), 2000, 0, "挂机检测");
            return true;
        }
                ...
                if (((!MapleLieDetector.this.isPassed()) /*&& (!isItem)*/) && (searchchr != null)) {
                    if (MapleLieDetector.this.attempt >= 3) {
                        ...
                        if ((search_chr != null) && (search_chr.getId() != searchchr.getId())) {
                            search_chr.dropMessage(5, searchchr.getName() + " 没有通过测谎仪的检测，恭喜你获得7000的金币.");
                        ...
                        World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] 玩家: " + searchchr.getName() + " (等級 " + searchchr.getLevel() + ") 未通过测谎仪检测，疑似使用脚本外挂！"));
                    } else {
