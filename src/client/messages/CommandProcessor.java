...
//                return ret;
                    ...
                    FileoutputUtil.logToFile(FileoutputUtil.CommandEx_Log, FileoutputUtil.NowTime() + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ")使用了指令 " + line + " ---在地图「" + c.getPlayer().getMapId() + "」频道：" + c.getChannel() + " \r\n");

                }
                return true;
                        }
                   int 游戏指令开关 = gui.ZeroMS_UI.ConfigValuesMap.get("游戏指令开关");
                   if (游戏指令开关 <= 0) {
                    ...
                    if (c.getPlayer() != null) {
                        ...
                        try {
                            ...
                            if (ret) {
                                ...
                                FileoutputUtil.logToFile("logs/Data/管理員命令.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 帳號: " + c.getAccountName() + " 玩家: " + c.getPlayer().getName() + " 使用管理員命令:" + line);
                                // 訊息處理
                                ShowMsg(c, line, type);
                            } else {
                                c.getPlayer().dropMessage("指令错误，用法： " + co.getMessage());
                            ...
                            if (c != null && c.getPlayer() != null) {
                                output += c.getPlayer().getName() + "(" + c.getPlayer().getId() + ")使用了指令 " + line + " ---在地图「" + c.getPlayer().getMapId() + "」頻道：" + c.getChannel();
                            }
    ...
    private static void ShowMsg(MapleClient c, String line, CommandType type) {
        ...
        if (c.getPlayer() != null) {
            ...
            if (c.getPlayer().getGMLevel() == 5) {
                ...
                System.out.println("＜ 荣誉主播 ＞ " + c.getPlayer().getName() + " 使用了指令: " + line);
