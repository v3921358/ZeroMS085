...
public class MapleTrade {
    ...
    public final void CompleteTrade() {
        ...
        if (exchangeItems != null) { // just to be on the safe side...
            ...
            if ((exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) >= 50000000) {
                FileoutputUtil.logToFile("logs/Data/大額金幣交易.txt", FileoutputUtil.NowTime() + "角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易獲得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + "\r\n");
                World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + "大額金幣交易 疑似賣遊戲幣 角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易獲得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso))));
            }

            FileoutputUtil.logToFile("logs/Data/交易記錄.txt", FileoutputUtil.NowTime() + " 帳號角色名字:" + chr.get().getClient().getAccountName() + " " + chr.get().getName() + " 和 " + partner.chr.get().getClient().getAccountName() + " " + partner.chr.get().getName() + " 交易獲得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + " 和 " + exchangeItems.size() + "件物品[" + output + "]\r\n");
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + "角色名字:" + chr.get().getName() + " 和 " + partner.chr.get().getName() + " 交易獲得 金幣" + (exchangeMeso - GameConstants.getTaxAmount(exchangeMeso)) + " 和 " + exchangeItems.size() + "件物品[" + output + "]"));
            exchangeItems.clear();
        }
        if (exchangeMeso > 0) {
        ...
        try {
            ...
            FileoutputUtil.logToFile("logs/交易存檔保存數據異常.txt", "\r\n " + FileoutputUtil.NowTime() + " IP: " + chr.get().getClient().getSession().remoteAddress().toString().split(":")[0] + " 帳號 " + chr.get().getClient().getAccountName() + " 帳號ID " + chr.get().getClient().getAccID() + " 角色名 " + chr.get().getName() + " 角色ID " + chr.get().getId());
            FileoutputUtil.outError("logs/交易存檔保存數據異常.txt", ex);
    ...
    public static final void inviteTrade(final MapleCharacter c1, final MapleCharacter c2) {
        int 玩家交易开关 = gui.ZeroMS_UI.ConfigValuesMap.get("玩家交易开关");
        if (玩家交易开关 > 0) {
