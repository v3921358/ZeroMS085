...
public class HiredMerchant extends AbstractPlayerStore {
    ...
    public void buy(MapleClient c, int item, short quantity) {
        ...
        if (MapleInventoryManipulator.addFromDrop(c, newItem, false)) {
            ...
            if (ServerConfig.LOG_MRECHANT) {
                FileoutputUtil.logToFile("logs/Data/精靈商人.txt", "\r\n 時間　[" + FileoutputUtil.NowTime() + "] IP: " + c.getSession().remoteAddress().toString().split(":")[0] + " 玩家 " + c.getAccountName() + " " + c.getPlayer().getName() + " 從  " + getOwnerName() + " 的精靈商人購買了" + MapleItemInformationProvider.getInstance().getName(newItem.getItemId()) + " (" + newItem.getItemId() + ") x" + quantity + " 單個價錢為 : " + pItem.price);
            }
    ...
    public void closeShop(boolean saveItems, boolean remove) {
        try {
            ...
            FileoutputUtil.outError("logs/精靈商人關閉異常.txt", se);
        }
    }
