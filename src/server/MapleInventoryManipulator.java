...
//            c.setMonitored(true);
        ...
        if (before == 0) {
            switch (item.getItemId()) {
                case AramiaFireWorks.XIANG_ID:
                    c.getPlayer().dropMessage(5, "你已经获得了一个香炉。");
                    break;
                case AramiaFireWorks.KEG_ID:
                    c.getPlayer().dropMessage(5, "你得到了一个永恒的雪花，可以在幸福村堆雪人。");
                    break;
                case AramiaFireWorks.SUN_ID:
                    c.getPlayer().dropMessage(5, "你得到了一个温暖的太阳，可以让枫树苗快速生长。");
                    break;
                case AramiaFireWorks.DEC_ID:
                    c.getPlayer().dropMessage(5, "你得到了一个圣诞树装饰，把这个交给白色圣诞山丘的黑鼻白毛，可以妆点圣诞树。");
                    break;
            }
        }
        c.getPlayer().havePartyQuest(item.getItemId());
    ...
    /*   MapleCharacter chr = c.getPlayer();
       if (chr.isGM()) {
       chr.dropMessage(5, "移动物品 " + source.getItemId() + "");
       }*/
        short olddstQ = -1;
        ...
        if (c.getPlayer().getDebugMessage() || c.getPlayer().isGM()) {
            c.getPlayer().dropMessage("移动道具: src : " + src + " dst : " + dst + " 代码：" + source.getItemId());
        }
        if( (source.getFlag() & 0x20) != 0){
            c.getPlayer().dropMessage("道具名称："+ii.getName(source.getItemId())+" 防爆保护中");
        }
    ...
    public static void equip(final MapleClient c, final short src, short dst) {
        ...
        if (source.getState() > 1) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    StructPotentialItem pot = MapleItemInformationProvider.getInstance().getPotentialInfo(i).get(MapleItemInformationProvider.getInstance().getReqLevel(source.getItemId()) / 10);
                    if (pot != null && pot.skillID > 0) {
                        c.getPlayer().changeSkillLevel_Skip(SkillFactory.getSkill(c.getPlayer().getStat().getSkillByJob(pot.skillID, c.getPlayer().getJob())), (byte) 1, (byte) 0);
                    }
                }
            }
        }
        ...
        if (reqlv > c.getPlayer().getLevel() + c.getPlayer().getStat().levelBonus && !c.getPlayer().isGM()) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改裝備(" + source.getItemId() + ")封包，穿上裝備時封鎖。 該裝備需求等級: " + reqlv));
    ...
    public static void unequip(final MapleClient c, final short src, final short dst) {
        ...
        if (source.getState() > 1) {
            ...
            for (int i : potentials) {
                if (i > 0) {
                    ...
                    if (pot != null && pot.skillID > 0) {
                        c.getPlayer().changeSkillLevel_Skip(SkillFactory.getSkill(c.getPlayer().getStat().getSkillByJob(pot.skillID, c.getPlayer().getJob())), (byte) 0, (byte) 0);
                    }
                }
            }
        }
        ...
        if (reqlv > c.getPlayer().getLevel() + c.getPlayer().getStat().levelBonus && !c.getPlayer().isGM()) {
            ...
            World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "[封鎖系統] " + c.getPlayer().getName() + " 因為修改封包而被管理員永久停權。"));
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語]  " + c.getPlayer().getName() + "(" + c.getPlayer().getId() + ") <等級: " + c.getPlayer().getLevel() + " > 修改裝備(" + source.getItemId() + ")封包，脫除裝備時封鎖。 該裝備需求等級: " + reqlv));
            c.getPlayer().ban("修改封包", true, true, false);
            c.getSession().close();
            return;
        }
    ...
    public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity, final boolean npcInduced, final boolean cs) {
        final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
     //   if (World.isShutDown) {
        int 丢出物品开关 = gui.ZeroMS_UI.ConfigValuesMap.get("丢出物品开关");
        if (丢出物品开关 > 0) {
        ...
        try {
            ...
            //这里检测数量是否负数复制BUG
              if (quantity < 0 || (!npcInduced && GameConstants.isPet(source.getItemId())) || (quantity == 0 && !GameConstants.isRechargable(source.getItemId()))) {
    ...
    public static void removeAllByEquipOnlyId(MapleClient c, long inventoryitemid) {
        ...
        if (copyEquipItems != null) {
            ...
            FileoutputUtil.log("Hack/複製裝備_已刪除.txt", msgtext + " 道具唯一ID: " + copyEquipItems.getEquipOnlyId());
        }
        ...
        if (copyEquipedItems != null) {
            ...
            World.Broadcast.broadcastGMMessage(MaplePacketCreator.serverNotice(6, "[GM密語] " + msgtext));
            FileoutputUtil.logToFile("Hack/複製裝備_已刪除.txt", msgtext + " 道具唯一ID: " + copyEquipedItems.getEquipOnlyId());
        }
    ...
    public static boolean removeById_fx(final MapleClient c, final MapleInventoryType type, final int itemId, final int quantity, final boolean fromDrop, final boolean consume) {
        int remremove = quantity;
        List<IItem> ret = c.getPlayer().getInventory(type).listById(itemId);
        for (int i = ret.size()-1; i >= 0; i--) {
            IItem item = ret.get(i);
            if (remremove <= item.getQuantity()) {
                removeFromSlot(c, type, item.getPosition(), (short) remremove, fromDrop, consume);
                remremove = 0;
                break;
