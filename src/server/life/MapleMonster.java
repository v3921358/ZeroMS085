...
public class MapleMonster extends AbstractLoadedMapleLife {
   ...
   /* public static int LevelExpA = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpA"));
    public static int LevelExpB = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpB"));
    public static int LevelExpC = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpC"));
    public static int LevelExpD = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpD"));
    public static int LevelExpE = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpE"));
    public static int LevelExpF = Integer.parseInt(ServerProperties.getProperty("ZeorMS.LevelExpF"));*/
    ...
    public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime, final int lastSkill) {
        ...
        if(damagei > from.getZdsh()){
            from.setZdsh(damagei);
            from.dropMessage("[ 仙境冒险岛 - 个人伤害系统 ]：目前最大伤害得到了更新.更新伤害值： " + damagei + "又强了一点~");
        }
     ...
     * @param lastskillID
     */
    private void giveExpToCharacter(final MapleCharacter attacker, long exp, final boolean highestDamage, final int numExpSharers, final byte pty, final byte classBounsExpPercent, final byte Premium_Bonus_EXP_PERCENT, final int lastskillID) {
        ...
        if (exp > 0) {
            ...
            if (attacker.getLevel() >= 10 && attacker.getLevel() <= 29) {//动态经验
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(1);
            } else if (attacker.getLevel() >= 30 && attacker.getLevel() <= 69) {
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(2);
            } else if (attacker.getLevel() >= 70 && attacker.getLevel() <= 119) {
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(3);
            } else if (attacker.getLevel() >= 120 && attacker.getLevel() <= 149) {
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(4);
            } else if (attacker.getLevel() >= 150 && attacker.getLevel() <= 199) {
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(5);
            } else if (attacker.getLevel() >= 200 && attacker.getLevel() <= 254) {
                exp *= ChannelServer.getInstance(map.getChannel()).getBounsExpRate(6);
            }                      
            //do this last just incase someone has a 2x exp card and its set to max value
            long classBonusExp = 0;
            if (classBounsExpPercent > 0) {
                classBonusExp = (long) (exp / 100.0) * classBounsExpPercent;
            }
            int premiumBonusExp = 0;
            if (Premium_Bonus_EXP_PERCENT > 0) {
                premiumBonusExp = (int) ((exp / 100.0) * Premium_Bonus_EXP_PERCENT);
            }
            long equpBonusExp = (long) ((exp / 100.0) * attacker.getStat().equipmentBonusExp);
            if (attacker.getStat().equippedFairy) {
                equpBonusExp += (exp / 100.0) * attacker.getFairyExp();
            }
            attacker.gainExpMonster(exp, true, highestDamage, pty, (int)classBonusExp, (int)equpBonusExp, premiumBonusExp);
 ...
 /* 召喚小弟囉 */
    ...
    public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison, long duration, final boolean checkboss, final MapleStatEffect eff) {
        ...
        if (gui.ZeroMS_UI.ConfigValuesMap.get("怪物状态开关") <= 0) {
        ...
        //    status.setPoisonDamage((int) (eff.getDamage() * from.getStat().getCurrentMaxBaseDamage() / 100.0), from);
            ...
            if (status.getchr() != null) {//修复忍者伏击开始
                return;
            }
            status.setDotTime(duration);
            int dam = (int) Math.min(Short.MAX_VALUE, (int) (eff.getDamage() * from.getStat().getCurrentMaxBaseDamage() / 100.0));
            if (from.hasGmLevel(5)) {
                from.dropMessage(6, "[持續傷害] 開始處理效果 - 技能ID：" + eff.getSourceId());
            }
            status.setValue(status.getStatus(), dam);
        ...
        } else if (NINJA_AMBUSH == 1) {
            poisonsLock.writeLock().lock();
            try {
                poisons.add(status);
                status.scheduledoPoison2(this);//修复忍者伏击结束
            } finally {
                poisonsLock.writeLock().unlock();
            }
        } else {
    ...
    public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        if ((status.getStatus() == MonsterStatus.VENOMOUS_WEAPON || status.getStatus() == MonsterStatus.POISON || status.getStatus() == MonsterStatus.NEUTRALISE || status.getStatus() == MonsterStatus.NINJA_AMBUSH) && poisons.size() <= 0) {//修复忍者伏击
            return;
        }
        if (status.getStatus() != MonsterStatus.VENOMOUS_WEAPON && status.getStatus() != MonsterStatus.POISON && status.getStatus() == MonsterStatus.NEUTRALISE && status.getStatus() == MonsterStatus.NINJA_AMBUSH && !stati.containsKey(status.getStatus())) {//修复忍者伏击
            return;
        }
        if (weakChr == null) {
            return;
    ...
    public final void doPoison2(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
        if ((status.getStatus() == MonsterStatus.VENOMOUS_WEAPON || status.getStatus() == MonsterStatus.POISON || status.getStatus() == MonsterStatus.NEUTRALISE || status.getStatus() == MonsterStatus.NINJA_AMBUSH) && poisons.size() <= 0) {
            return;
        }
        if (status.getStatus() != MonsterStatus.VENOMOUS_WEAPON && status.getStatus() != MonsterStatus.POISON && status.getStatus() == MonsterStatus.NEUTRALISE && status.getStatus() == MonsterStatus.NINJA_AMBUSH && !stati.containsKey(status.getStatus())) {
            return;
        }
        if (weakChr == null) {
            return;
