...
public class AramiaFireWorks {
    ...
    public final static int MAX_KEGS = 5000, MAX_SUN = 3000, MAX_DEC = 18000;
    ...
    public final void giveKegs(final MapleCharacter c, final int kegs) {
        ...
        switch (this.kegs) {
            case 1000:
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<頻道 " + c.getClient().getChannel() + "> " + "不夜城新年活動進度目前是5000/" + this.kegs + "！！"));
                break;
            case 2000:
                World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, "<頻道 " + c.getClient().getChannel() + "> " + "不夜城新年活動進度目前是5000/" + this.kegs + "！！"));
                break;
    ...
    private void broadcastServer(final MapleCharacter c, final int itemid) {
        World.Broadcast.broadcastMessage(MaplePacketCreator.serverNotice(6, itemid, "<频道 " + c.getClient().getChannel() + "> " + "枫树山丘的枫树正在阳光下绽放！！"));
    }
    ...
    public final short getSunsPercentage2() {
    ...
    private void startSun(final MapleMap map) {
        map.startMapEffect("阳光正在绽放！", 5121010);
        for (int i = 0; i < 3; i++) {
            EventTimer.getInstance().schedule(new Runnable() {
    ...
    private void spawnItem(final MapleMap map) {
        ...
        for (int i = 0; i < Randomizer.nextInt(5) + 10; i++) {
            ...
            map.spawnAutoDrop(Randomizer.nextInt(10) == 1 ? 3010025 : 4001246, pos);
            if (Randomizer.nextInt(10) == 3) {
                map.spawnAutoDrop(4000313, pos);
                map.spawnAutoDrop(4001126, pos);
            }
        }
    }
