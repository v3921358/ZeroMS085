...
public class DumpNpcNames {
    ...
    public void dumpNpcNameData() throws SQLException {
        ...
        try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("DELETE FROM `wz_npcnamedata`")) {
            ps.execute();
        }
        ...
        for (int key : npcNames.keySet()) {
            try {
                try (Connection con = DatabaseConnection.getConnection(); PreparedStatement ps = con.prepareStatement("INSERT INTO `wz_npcnamedata` (`npc`, `name`) VALUES (?, ?)")) {
                    ps.setInt(1, key);
