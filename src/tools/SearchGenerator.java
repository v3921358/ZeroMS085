...
public class SearchGenerator {
    ...
    public static String searchData(SearchType type, String search) {
        ...
        if (!sb.toString().isEmpty() && !sb.toString().equalsIgnoreCase("对不起, 搜索内容不存在")) {
            sbs.append("<<类型: ").append(type.name()).append(" | 搜索信息: ").append(search).append(">>");
        }
        sbs.append(sb);
        if (sbs.toString().isEmpty()) {
            sbs.append("搜搜索不到").append(type.name());
        }
        return sbs.toString();
    }
